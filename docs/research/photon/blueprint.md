...
资料截至：2026-09-12。对应发布版本：PF **BETA 0.1／2026.09.10-r3**，PM **BETA 0.1.1／2026.09.11-r4**。
...
**2026-07-07** 的 PF quick-save 试验记录了 RIO 扩写、实机写入与字节回读，是已保存的早期资源修改实例。
...
PF 那一条非 CRmt 映射的 source key 为 `0xA3256C75`，111,236 字节；严格 Cr6Ti 解码结果为完整封装、1024×600、kind2。Cr6Ti 与 CRip008 共享 `00 04 45` 对象前缀，**不能只看这三个字节判断格式**。
...
| T13 | 特殊文本前缀／普通字体修复 | **C，修复项目回归** | PM 297 条 U+0010 指令残缺，最终统一为普通字体；只改七个 CRsa，PF 无同类受影响项。 |
...
1. **两份正式清单**：验证对象为 PF r3、PM r4 的正式包清单。
2. **实际 RUO 文件**：完整文件 hash 与包清单一致，解析全部映射；50 个 CRmt 载荷匹配历史确认记录 hash；PF 剩余一个 Cr6Ti 用严格解码器解析和解码。
3. **最终差分里的追加区**：直接从补丁 segment 重构每个新增字节，证明完整覆盖；PF、PM 历史追加前缀 hash 均匹配。
4. **最终文本清单**：164 个固定范围 CRsa、57,723 条；r3 回看修复与 r4 七记录修复另由对应构建／发布记录衔接。
5. **GitHub 发布身份**：两个 Release 均 `prerelease=false`；PF ZIP 为 112,873,626 字节，PM ZIP 为 152,585,277 字节。
...
发布页：[PF BETA 0.1](https://github.com/imnotsureyi-sys/muvluv-series-steam-cn-patch/releases/tag/pf-BETA-0.1)、[PM BETA 0.1.1](https://github.com/imnotsureyi-sys/muvluv-series-steam-cn-patch/releases/tag/pm-BETA-0.1.1)。PF GitHub UTC 发布时间为 9 月 9 日 17:36:54，对应悉尼 9 月 10 日凌晨。
