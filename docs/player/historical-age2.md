...
> 以下旧 ZIP 已被新版替代，仅保留历史说明。请优先使用[当前下载与安装指南](README.md)。


以下表格与后续安装、备份、回滚路径说明适用于 TDA00—03 和帝都燃烧篇；光子之花 / 光子旋律请使用[当前版本指南](README.md)中的独立流程。
...
本页仅供旧版问题追溯；新安装请使用[当前版本指南](README.md)。不要安装已被取代的 `tda01-beta0.1`、`tda01-beta0.2`、`tda01-beta0.2.1` 或 `tda03-beta0.1`；`tda01-beta0.2` 会产生 603 个不可见正文槽，`tda03-beta0.1` 误用了 TDA02 的 UI/成就映射。
