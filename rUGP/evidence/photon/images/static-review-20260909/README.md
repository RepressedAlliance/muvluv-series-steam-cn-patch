...
[第一轮用户实机采集](runtime-capture-round-01.json) 记录光子之花 95 条、光子旋律 14 条
普通绑定的提交回读成功，光子旋律另有 4 条仅观察到加载。两款已恢复正常 DLL；
...
- 四组返回按钮为光子之花粉色 G488/G490、青色 G489/G491，光子旋律橙色 G1003/G1005、
  绿色 G1004/G1006；本地修订统一字形和位置，保留状态颜色差异。
- 光子旋律启动署名在本地绑定到 G1281 对应资源。公开源码现已同步图片 hook 准入、
...
完整故障说明见 [光子旋律图片准入修复](../../../../docs/postmortems/pm-image-admission-20260909.md)。
...
2 项 Logo 仍只有映射关系；光子旋律 3 项仅证明预加载，1 项另有带状态判断的壁纸解锁
调用。40 项均不是已确认故障，44 项的实机待验证状态没有因静态引用而升级。

[第二轮光子之花开场采集](runtime-capture-round-02.json) 记录 578 个事件、78 个普通
绑定成功提交并回读；目标兵装图片身份仍未观察到。用户补充的日文图片模式截图
同样显示英文 HUD，现仅登记实际素材对应关系待核验，撤回已确认替换失效的口头
判断。不将它计为通过、已确认 hook 故障或补齐故障。光子之花正常 DLL 已恢复；
...
另外四个保留的光子旋律身份未读取图片。初次审计未修改安装目录；随后依据用户的
光子旋律相册崩溃转储修复语言绑定失效处理，并根据光子之花祭典实机日志修复缓冲区补齐与
...
两款当前构建记录均已更新，光子之花旧版本身份在 `pf_festival_fix` 中保留。
[光子之花祭典演出修复](../../../../docs/postmortems/pf-festival-presentation-20260909.md)
...
| 游戏 | 默认构建证据 | 实机配置重建证据 |
| --- | --- | --- |
| 光子之花 | [pf-default.build.json](pf-default.build.json) | [pf-installed.build.json](pf-installed.build.json) |
| 光子旋律 | [pm-default.build.json](pm-default.build.json) | [pm-installed.build.json](pm-installed.build.json) |

光子之花安装时实际使用的 `headers` 与其 `source/generated` 副本不同，本次采用前者。
两款 DLL 均编入了跨游戏身份，因此光子旋律表保留按编译游戏选择的封存视图：光子之花版保留
1,594 个光子旋律身份，光子旋律版为 1,598 个。不能把一个目录里较新的所有文件一并当作
