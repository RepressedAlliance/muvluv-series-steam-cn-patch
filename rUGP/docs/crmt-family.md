...
本指南针对已观察、验证的光子之花／光子旋律 Steam 资源格式。工具遇到未知布局应拒绝，不把它泛化为所有 AGES 游戏支持。
...
例如 U0478 有两个光子旋律目标；日文竖排 4 层、英文横排 6 层、已批准中文竖排 4 层。
不能按层序跨语言强行认定相同尺寸，也不能把某一个目标通过当成两个目标都通过。
光子之花／光子旋律的 U0006 则是不同物理资源，哪一个被请求和替换必须分别核对。
...
- 光子旋律诊断曾依赖 8 GiB RUO 地址空间修正。诊断启动器通过不自动说明正式 DLL 通过；必须分别锁定 DLL、RUO 与 EXE 身份验证。
