# 光子旋律图片 hook 已安装但图片功能关闭

光子旋律原先只有 7 个普通 hook。补入 CRip008 的两个直接解码调用点后，底层安装数
变为 9；production 层仍按 7 验收，把有效初始化当作失败并关闭图片替换。
用户看到菜单和图片 UI 仍为英文，单看底层 hook 成功日志会漏掉这个问题。

本次把光子旋律普通表、直接解码调用包装和两个调用点身份一起纳入公开源码，并将
production 准入数同步为 9。调用包装沿用光子之花的 ABI，保留原解码调用与寄存器，
只在受认证的 sidecar 和 surface transaction 条件满足时提交替换。
光子之花默认调用点签名字节不变；后续同步安装表后，两款默认构建的身份均已更新。
...
和 surface 提交；不伪造 native 成功状态，不绕过 selector。旧版只回退光子旋律
...
实机光子旋律 DLL；具体两种构建身份见 [运行时说明](../../runtime/README.md)。图片文件仍在
