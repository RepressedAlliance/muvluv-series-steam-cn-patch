...
- `crsa_vm_stream.py` and `crsa_vm_schema.json`: sequential, read-only 光子之花/光子旋律 native command/object parsing using executable-hash-bound field descriptors, including counted strings, call arguments, message pools and trailing references. Unknown layouts fail without byte resynchronization.
...
- `crsa_vm_edit.py`: hash-bound 光子之花/光子旋律 message and annotation edits. It prefers native slots and unused zero runs; non-empty orphan reuse or pool growth requires an explicit reviewed storage contract. It exposes no armament-parameter write action.
...
The public [CRsa extraction CLI](../../tools/text/README.md) uses these readers against extents from the ICI/RIO catalog. Its hash-only output is an unreviewed translation template, not proof that a slot has a safe static writer or runtime binding. The separate native-field auditor consumes a hash-checked full-volume scan cache; see the [光子之花/光子旋律 full-field audit](../../docs/postmortems/crsa-native-text-20260903.md) for its completed static coverage and remaining runtime boundary.
