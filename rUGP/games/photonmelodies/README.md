...
光子旋律的 CRsa 路线暴露了 AGES Internal Error 8311：带长度的 CString 内混入 `U+0000`
会被运行时拒绝，即使静态结构和校验和都正确。根因与修复保存在
[8311 复盘](../../docs/postmortems/error-8311.md)。图片使用光子之花/光子旋律共用清单，但光子旋律的
...
因此光子旋律原生 CRsa 不再由 RUO 构建器输出，而由
...
当前组件还不是玩家安装包。正式发布需要光子旋律自己的干净根构建、哈希门、字体与图片
权利核验、安装/卸载以及完整路线实机 QA。

## English summary

光子旋律 has 44,698 latest review entries in chapter CSVs, a sealed legacy
